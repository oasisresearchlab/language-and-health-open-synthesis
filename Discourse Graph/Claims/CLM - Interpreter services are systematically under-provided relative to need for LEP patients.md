---
shortLabel: "Interpreter under-provision"
NodeFormality: draft
TruthValue: 0.8
NodeType: Claim
nodeTypeId: node_nMxzA_OByPwgPcmb6AN82
curationStatus: Initial AI draft
nodeInstanceId: 019ebef0-8837-741d-a02a-a44ed1b642eb
tags:
  - languageConcordanceFactor/concordanceIntervention/interpretingServices
  - healthOutcome/serviceUtilization
  - epistemic/measurement
languageConcordanceFactor:
  - Interpretation services
  - Limited English Proficiency (LEP)
healthOutcome:
  - Service utilization
deliveryContext:
  - Inpatient
  - Surgery
  - Emergency Department
---
## Supporting Evidence

> [!info] EVDs that support this claim (→ EVD—supports→CLM edges by sync_relations.py). Measured rates of interpreter offer/use fall well below the population that needs concordant communication.

- [[EVD - Interpreter or bilingual-provider access was offered to only 24% of postpartum couplets at admission and 14% for daily rounds - @Jennifer_2023_Evaluation_Patient]]
- [[EVD - Interpreter use was documented for only 62% of LEP cholecystectomy patients despite the Section 1557 mandate - @J_2025_Pilot_study]]
- [[EVD - About half (54.4%) of admitted LEP patients requiring an interpreter received one - @Morris_2021_Factors_associated]]
- [[EVD - Female LEP patients were more likely than males to receive an interpreter (65.1% vs 41.7%) - @Morris_2021_Factors_associated]]
- [[EVD - Interpreters were requested for 12.1% of pediatric ED patients in 2016, up from ~2% in 2002 - @Greenky_2019_Reversed_Trend]]

## Contradicting Evidence

> [!info] EVDs that oppose this claim (→ EVD—opposes→CLM) — measures suggesting provision is approaching or exceeding need.

(none identified in the current corpus)

## Other Notes

Across distinct settings and countries — US postpartum inpatient (Jensen), US surgical (Rosenthal), Australian general inpatient (Morris), and US pediatric emergency (Greenky) — measured interpreter offer/use for LEP patients who require concordant communication clusters far below universal: roughly 14–24% of routine encounters offered access (Jensen), 62% of surgical-consent encounters documented (Rosenthal), and only ~54% of audited inpatients who needed an interpreter received one (Morris). The pattern is one of chronic under-provision relative to need rather than incidental gaps. Morris additionally finds the only patient characteristic predicting receipt is sex (females favored), indicating provision is not even uniformly distributed within the eligible population — a sign that allocation is driven by factors other than clinical need. Greenky's rise in interpreter requests from ~2% (2002) to 12.1% (2016) shows utilization is increasing over time, but the absolute prevalence still reflects a population whose need is only partially met. Several of these are descriptive measurement studies, so they document the size of the unmet-need gap rather than its downstream effects.

> "This study found just over half (54.4%) of admitted patients who required a health care interpreter received one, which exceeds results reported by other recent Australian studies." (Morris, 2021, p. 429)
