...
async function readNodeFile(filePath: string): Promise<GraphNode | null> {
  ...
  return {
    ...
    author: toStringMaybe(fm.author),
    year: toStringMaybe(fm.year),
    journal: toStringMaybe(fm.journal),
    doi: toStringMaybe(fm.doi),
    pubmedId: toStringMaybe(fm.pubmedId),
    citekey: toStringMaybe(fm.citekey),
